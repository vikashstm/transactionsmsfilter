...
class SBIUPIBank {
    ...
    fun getPayTo(sender: String, message: String): String {
        var payName = ""
        if (sender.contains("SBIUPI", true)) {
            if (message.contains("UPI", true)) {
                if (message.contains("RefNo", true)) {
                    // split from :
                    var data = message.split("RefNo")
                    if (data.isNotEmpty()) {
                        payName=data[1].trim().split(".")[0]
                    }
                }else if (message.contains("Ref No", true)) {
                    // split from :
                    var data = message.split("Ref No")
                    if (data.isNotEmpty()) {
                        payName=data[1].trim().split(".")[0]
                    }
                }else if (message.contains("on", true)) {
                    // split from :
                    var data = message.split("on")
                    if (data.isNotEmpty()) {
                        payName=data[1].trim().split("Download")[0]
                    }
                }
